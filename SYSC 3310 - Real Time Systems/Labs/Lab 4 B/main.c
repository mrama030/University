...
void Port2_Output(uint8_t color){  // write three outputs bits of P2
  P2OUT = (P2OUT & 0xF8) | color;
}

void PORT1_IRQHandler(void) {
  FallingEdges = FallingEdges + 1;

	// Switch 1 (P1.1) Interrupt received:
	if ((P1IFG & 0x02) != 0) 
	{
		P1IFG &= ~0x02;		// clear flag1, acknowledge
    Port2_Output(BLUE);   // toggle LED1
  }

	// Switch 1 (P1.4) Interrupt received:
	if ((P1IFG & 0x10) != 0) 
	{
		P1IFG &= ~0x10;		// clear flag4, acknowledge
		Port2_Output(RED);   // toggle LED1
  }
}
